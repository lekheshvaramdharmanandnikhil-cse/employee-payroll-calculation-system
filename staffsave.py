...
def staffsave() :
    ...
    d.place(x=0,y=0)
    head=Label(t,text='Staff Save',font=('arial',35,"italic underline"),fg='black',bg='silver')
    head.place(x=350,y=10)
    def checkdata():
        db=pymysql.connect(host='localhost',user='root',password='root',database='epcs')
        cur=db.cursor()
        xa=int(e1.get())
        sql="select count(*) from staff where staffid=%d" %(xa)
        cur.execute(sql)
        data=cur.fetchone()
        if data[0]==0:
            mes.config(fg='Green')
    ...
    def savedata():
        ...
        if len(e1.get())==0 or len(e2.get())==0 or len(e3.get())==0 or len (e4.get())==0 or len (e5.get())==0 or len(e6.get())==0:
            ...
            xe=e5.get()
            xf=int(e6.get())
            sql="insert into staff values(%d,'%s','%s','%s','%s',%d)"%(xa,xb,xc,xd,xe,xf)
            cur.execute(sql)
            db.commit()
    ...
    b=Label(t,text='SName',font=('georgia',25,"italic underline"),fg='black',bg='silver')
    b.place(x=300,y=190)
    ...
    f=Label(t,text='Dept-No',font=('georgia',25,"italic underline"),fg='black',bg='silver')
    f.place(x=300,y=550)
    e6=Entry(t,width=20,bd=15)
    e6.place(x=500,y=550)
    
    g=Label(t,text='D',font=('georgia',25,"italic underline"),fg='black',bg='silver')
    g.place(x=300,y=550)
