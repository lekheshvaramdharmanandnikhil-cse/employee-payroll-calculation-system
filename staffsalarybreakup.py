...
def staffsalarybreakupdelete() :
    t=tkinter.Tk()
    t.geometry('1300x750')
    d=Canvas(t,height=1500,width=1500,bg='violet')
    d.place(x=0,y=0)
    head=Label(t,text='Staff Salary Breakup Delete',font=('arial',30,"italic underline"),fg='black',bg='white')
    head.place(x=400,y=10)
    def filldata() :
        db=pymysql.connect(host='localhost',user='root',password='root',database='epcs')
        cur=db.cursor()
        lt=[]
        sql="select staffid from staffsalarybreakup"
        cur.execute(sql)
        data=cur.fetchall()
        for res in data:
            lt.append(res[0])
    ...
    def deldata() :
        ...
        cur=db.cursor()
        xa=int(e1.get())
        sql="delete from staffsalarybreakup where staffid=%d" %(xa)
        cur.execute(sql)
        db.commit()
    ...
    a=Label(t,text='Staffid',font=('georgia',30,"italic underline"),bg='light pink',height=1,width=7)
    a.place(x=400,y=160)
    e1=ttk.Combobox(t) 
    filldata()
