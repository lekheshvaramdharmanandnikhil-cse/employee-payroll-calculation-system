...
def companydatadelete() :
    t=tkinter.Tk()
    t.geometry('1300x750')
    d=Canvas(t,height=1500,width=1500,bg='violet')
    d.place(x=0,y=0)
    head=Label(t,text='Company Data Delete',font=('arial',30,"italic underline"),fg='black',bg='white')
    ...
    def deldata() :
        ...
        cur=db.cursor()
        xa=int(e1.get())
        sql="delete from companydata where comid=%d" %(xa)
        cur.execute(sql)
        db.commit()
    ...
    a=Label(t,text='compid',font=('georgia',30,"italic underline"),bg='light pink',height=1,width=7)
    a.place(x=400,y=160)
    e1=ttk.Combobox(t) 
    filldata()
